            ...
            <div class="card card-statistics h-100">
                <div class="card-body">
                    <div class="col-xl-12 mb-30">
                        <div class="card card-statistics h-100">
                            <div class="card-body">
                                <a href="{{route('subjectCreate')}}" class="btn btn-success btn-sm" role="button"
                                ...
                                <div class="table-responsive">
                                    <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                                        ...
                                        @foreach($subjects as $subject)
                                            ...
                                            <td>{{$subject->$field}}</td>
                                            <td>{{$subject->grade->$field}}</td>
                                            <td>{{$subject->classroom->$field}}</td>
                                            <td>{{$subject->teacher->$Field}}</td>
                                                <td>  
                                                    <a href="{{route('subjectEdit',$subject->id)}}" class="btn btn-info btn-sm" role="button" aria-pressed="true"><i class="fa fa-edit"></i></a>
                                            ...
                                            <div class="modal fade" id="delete_subject{{$subject->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <form action="{{route('subjectDelete')}}" method="post">
                                                     @csrf     
                                                     @method("POST") 
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <p> {{ trans('classroom_list.warning_delete_classroom') }} <br/> {{$subject->$field}}</p>
                                                            <input type="hidden" name="id"  value="{{$subject->id}}">
                                                        </div> 
                                                        <div class="modal-footer">
                                                            <div class="modal-footer">
                                                                <button type="button" class="btn btn-secondary"
                                                                        data-dismiss="modal">{{ trans('grades_list.cancel') }}</button>
                                                                <button type="submit"
                                                                        class="btn btn-danger">{{ trans('grades_list.submit_delete_grade') }}</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    </form>
