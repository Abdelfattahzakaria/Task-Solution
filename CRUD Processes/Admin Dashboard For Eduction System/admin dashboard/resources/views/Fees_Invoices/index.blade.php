                                ...
                                <div class="table-responsive">
                                    <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                                        ...
                                        @foreach($Fee_invoices as $Fee_invoice)
                                            ...
                                            <td>{{$Fee_invoice->student->$field}}</td> 
                                            <td>{{$Fee_invoice->fee->$title}}</td>
                                            <td>{{ number_format($Fee_invoice->amount, 2) }}</td>
                                            <td>{{$Fee_invoice->grade->$field}}</td>
                                            <td>{{$Fee_invoice->classroom->$field}}</td>
                                            <td>{{$Fee_invoice->description}}</td> 
                                                <td> 
                                                    <a href="{{route('feesInvoicesEdit',$Fee_invoice->id)}}" class="btn btn-info btn-sm" role="button" aria-pressed="true"><i class="fa fa-edit"></i></a>
